...
import { findWallpaperWithLikeStatus } from '@/actions/wallpaper-actions';
...
export default async function Page({ params }: { params: { id: string } }) {
  const { id: wallpaperId } = params;
  const res = await findWallpaperWithLikeStatus(wallpaperId);
      ...
      <WallpaperCardContent className='space-y-4'>
        ...
        <div className='space-y-3'>
          ...
          <div className='flex gap-x-2'>
            <Suspense
              fallback={<SkeletonList length={7} className='h-6.5 w-36' />}
            >
              <WallpaperTags promise={findTagsByWallpaperId(wallpaper.id)} />
            </Suspense>
          </div>
        </div>
      </WallpaperCardContent>
